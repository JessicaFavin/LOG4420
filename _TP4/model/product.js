...
module.exports.createProduct = function(callback, id, name, price, image, category, description, features) {
    ...
    Product.getProductById(function(product) {
        if(product) {
            ...
            var featuresContentNotEmpty = true;
            for (var feature of features) {
              if(typeof feature !== 'string'){
                  featuresContentType = false;
              }
              if(feature === "") {
                  featuresContentNotEmpty = false
              }
            }
            //check that everything is true
            if(!idExists && idType && nameType && nameNotEmpty && priceType
                && imageType && imageNotEmpty && categoryType && categoryNotEmpty
                && categoryIsValid && descriptionType && descriptionNotEmpty
                && featuresType && featuresContentType && featuresContentNotEmpty){
...
module.exports.deleteProductById = function(callback, id) {
    Product.getProductById(function(product) {
        if(product) {
            Product.remove({'id':id}, function(error) {
                if(error) {
                    throw error;
                }
                callback(204);
